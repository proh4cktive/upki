...
class Authority(upkica.core.Common):
    ...
    def initialize(self, keychain=None):
        ...
        try:
            ...
            raise upkica.core.UPKIError(3, 'Unable to setup config: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(4, err)
        ...
        try:
            ...
            raise upkica.core.UPKIError(6, 'Unable to initialize CA Private Key: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(10, 'Unable to initialize CA Public Certificate: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(12, 'Unable to activate server: {e}'.format(e=err))
    ...
    def __import_keychain(self, profile, ca_path):
        ...
        if not os.path.isdir(ca_path):
            raise upkica.core.UPKIError(7, 'Directory does not exists')
        ...
        if csr_pem is None:
            try:
                self._request.generate(priv_key, "CA", profile)
            except Exception as err:
                raise upkica.core.UPKIError(6, err)
    ...
    def __create_keychain(self, profile):
        ...
        try:
            ...
            raise upkica.core.UPKIError(6, 'Unable to generate CA Private Key: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(7, 'Unable to store CA Private key: {e}'.format(e=err))

        try:
            cert_req = self._request.generate(priv_key, "CA", profile)
        except Exception as err:
            raise upkica.core.UPKIError(8, 'Unable to generate CA Certificate Request: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(9, 'Unable to store CA Certificate Request: {e}'.format(e=err))

        try:
            pub_cert = self._public.generate(cert_req, None, priv_key, profile, ca=True, selfSigned=True)
        except Exception as err:
            raise upkica.core.UPKIError(10, 'Unable to generate CA Public Certificate: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(11, 'Unable to store CA Public Certificate: {e}'.format(e=err))
    ...
    def __create_listener(self, profile, pub_cert, priv_key):
        ...
        try:
            ...
            raise upkica.core.UPKIError(14, 'Unable to generate Server Private Key: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(15, 'Unable to store Server Private key: {e}'.format(e=err))

        try:
            server_cert_req = self._request.generate(server_priv_key, "ca", server_profile)
        except Exception as err:
            raise upkica.core.UPKIError(16, 'Unable to generate Server Certificate Request: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(17, 'Unable to store Server Certificate Request: {e}'.format(e=err))

        try:
            server_pub_cert = self._public.generate(server_cert_req, pub_cert, priv_key, server_profile)
        except Exception as err:
            raise upkica.core.UPKIError(18, 'Unable to generate Server Public Certificate: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(19, 'Unable to store Server Public Certificate: {e}'.format(e=err))
    ...
    def load(self):
        ...
        if not os.path.isfile(self._config._path):
            raise upkica.core.UPKIError(21, "uPKI is not yet initialized. PLEASE RUN: '{p} init'".format(p=sys.argv[0]))
        
        try:
            self.output('Loading config...', level="DEBUG")
            self._config.load()
        except Exception as err:
            raise upkica.core.UPKIError(21, 'Unable to load configuration: {e}'.format(e=err))
        ...
        try:
            ...
            raise upkica.core.UPKIError(22, 'Unable to connect to db: {e}'.format(e=err))
    ...
    def register(self, ip, port):
        ...
        try:
            ...
            raise upkica.core.UPKIError(23, 'Unable to generate seed: {e}'.format(e=err))

        if not validators.ipv4(ip):
            raise upkica.core.UPKIError(24, 'Invalid listening IP')
        if not validators.between(int(port), 1024, 65535):
            raise upkica.core.UPKIError(25, 'Invalid listening port')
        ...
        try:
            ...
            raise upkica.core.UPKIError(26, 'Unable to initialize register: {e}'.format(e=err))
    ...
    def listen(self, ip, port):
        ...
        if not validators.ipv4(ip):
            raise upkica.core.UPKIError(27, 'Invalid listening IP')
        if not validators.between(int(port), 1024, 65535):
            raise upkica.core.UPKIError(28, 'Invalid listening port')
        ...
        try:
            ...
            raise upkica.core.UPKIError(29, 'Unable to initialize listener: {e}'.format(e=err))
